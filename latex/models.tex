...
\newpage
 ...
 \end{eqnarray}
  ...
  \int \limits_{V} a \frac{\partial S^w}{\partial t} d V - \oint \limits_{\Omega} b^{n}S^w d\vec{\Omega} = 0,
  \end{eqnarray}
  ...
  S^g = 1 - S^w,
  \end{eqnarray}
  where  $S$ is saturation, $w$~--~water, $g$~--~gas, $b$ can be treated as a velocity of fluid mixture inside the throat. 
 \newpage
The following initial (\ref{eq:twophase_saturation_init}) and boundary conditions (\ref{eq:twophase_saturation_boundary}) have been applied:
  ...
  \end{eqnarray}
  where $\Delta V$~--~volume of a particular grid-block, $\Delta t$~--~numerical time step, $a = \mathit{1}$, $b^n = v^n$, $\overline{b}$~--~average $b$, $\Delta L$~--~distance between centres of two neighbouring grid-blocks, $\Delta S$~--~surface bounding a grid-block (positive or negative sign is defined by the normal direction).
